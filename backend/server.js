...
const corsOrigin = (origin, callback) => {
  ...
  // not allowed -> do not throw an error (which would result in a 500).
  // Returning (null, false) means the CORS middleware will disallow the origin
  // and not set CORS headers.
...
// Small request logger to help debug CORS/preflight issues (remove in production if you want)
app.use((req, res, next) => {
  console.log(`[${new Date().toISOString()}] ${req.method} ${req.originalUrl} Origin:`, req.headers.origin || 'none');
  next();
});

// --- Middleware: CORS Configuration (safer) ---
app.use(cors({
  origin: corsOrigin,
...
// Ensure OPTIONS preflight is handled (use same CORS policy)
...
// Routes
// Mount both variants so frontend can call either /auth/... or /api/auth/...
...
app.use((err, req, res, next) => {
  ...
  // For CORS origin callback, if we returned false, the cors middleware will not set CORS headers.
  // If we still get here due to an error, return 500 with safe message.
  res.status(500).json({
    success: false,
